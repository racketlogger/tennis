...
class Tennis
  ...
  def process_score(score, best_of=3)
    begin
      ...
      fail "invalid number of sets" unless (2..best_of).cover? sets.length
      ...
      fail "nobody won" if home + away == 0
      @winner = home > away ? 0 : away > home ? 1 : fail('no winner')
      # sets won and lost
  ...
  def map_scores_winners(sets)
    sets.map do |set|
      ...
      fail "uneven games in set '#{set}'" unless games.length == 2
      h, a = games
      sw = set_winner(h, a, set == sets.last)
      fail "no valid winner in set '#{set}'" unless sw
      [games, sw]
    end
  end
  ...
  def set_winner(h, a, last_set = false)
    if last_set
      return nil unless h > 5 || a > 5
      # either tie-break or else difference of one+
      return nil unless h + a == 13 || h > a + 1 || a > h + 1
      return nil if h + a > 13 && (h-a).abs != 2
      return h > a ? 0 : 1
    end
    # basic range check
    return nil if h > 7 || a > 7 || h < 0 || a < 0
    # game went to 7
    return 0 if h == 7 && [5, 6].include?(a)
    return 1 if a == 7 && [5, 6].include?(h)
    # there has to be one winner, to 6
    return nil unless (h == 6 && h > a + 1) || (a == 6 && a > h + 1)
    h > a ? 0 : 1
  end
